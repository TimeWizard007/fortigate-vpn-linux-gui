...
The current version is **0.1.0** (foundation only).

| Capability | Status |
| ---------- | ------ |
| Application window and navigation | Implemented (placeholder UI) |
...
The **Connect with SSO** button is a placeholder. It does not open a browser,
authenticate, or change your network.

## Intended architecture (planned)

SAML authentication is expected to use the user's system browser and Microsoft
Entra ID. The following stack is the design target; **none of the layers below
the GUI exist yet**:
...
`openfortivpn` is **not** required for v0.1.x.
