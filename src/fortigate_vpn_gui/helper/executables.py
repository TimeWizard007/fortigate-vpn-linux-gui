...
``--help`` / ``--version``, not from assuming a path.
"""
...
def discover_approved_openfortivpn(
    *,
    ...
    for path in extra_paths:
        if not is_approved_openfortivpn_path(path) and not is_openfortivpn_executable(path):
            continue
        if path not in APPROVED_OPENFORTIVPN_PATHS:
            continue
        if not is_executable(path):
            continue
        source = "usr_local" if path.startswith("/usr/local/") else "usr_bin"
...
def resolve_approved_executable(
    *,
    ...
    discovered = discover_approved_openfortivpn(is_executable=is_executable)
    if not discovered:
        return None
    candidates = tuple(
