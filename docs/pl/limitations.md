...
Wersja v0.1.x to wydanie fundamentu. **Nie** jest działającym klientem VPN.

- Brak połączenia VPN: `openfortivpn` nie jest uruchamiany.
- Brak SAML/SSO: logowanie Microsoft Entra ID jest planowane, nie zaimplementowane.
- Brak przechowywania profili: selektor profili to wyłączony element zastępczy.
- Brak pomocnika uprzywilejowanego, reguł polkit i usług systemd.
- Brak zmian tras, DNS i zapory.
- Brak przechowywania haseł i tokenów.
...
- Kontrolka GUI „Connect with SSO” jedynie wyjaśnia, że funkcji jeszcze nie ma.
