...
def test_popen_passes_strongswan_conf_environment() -> None:
    captured: dict[str, object] = {}
    fake = MagicMock()
    fake.pid = 99
    fake.poll.return_value = 0
    ...
    with patch("fortigate_vpn_gui.vpn.process.subprocess.Popen", side_effect=popen):
        process = SubprocessVpnProcess(
            ...
            lambda _line: None,
            lambda _code: None,
            env={"STRONGSWAN_CONF": "/tmp/run/strongswan.conf", "PATH": "/usr/bin"},
        )
        process.start()
        if process._reader is not None:
            process._reader.join(timeout=2)

    assert captured["argv"] == ["/usr/lib/ipsec/charon"]
    kwargs = captured["kwargs"]
    assert isinstance(kwargs, dict)
    assert kwargs["env"]["STRONGSWAN_CONF"] == "/tmp/run/strongswan.conf"
    assert "--conf" not in captured["argv"]
